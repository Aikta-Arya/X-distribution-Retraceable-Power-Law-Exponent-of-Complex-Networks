function h=plplot(x, xmin, alpha,t1)
...
switch f_dattype,
    ...
    case 'REAL',
        ...
        %figure;
        h(1) = loglog(c(:,1),c(:,2),'bo','MarkerSize',5,'MarkerFaceColor',[1 1 1]); hold on;
    ...
    case 'INTS',
        ...
        %figure;
        h(1) = loglog(c(:,1),c(:,2),'b o','MarkerSize',5,'Color',[0 0 1]); hold on;
       % h(2) = loglog(cf(:,1),cf(:,2),'-','LineWidth',2,'Color',[0 0 0]); hold on;
        xr  = [10.^floor(log10(min(x))) 10.^ceil(log10(max(x)))];
        xrt = (round(log10(xr(1))):2:round(log10(xr(2))));
        if length(xrt)<4, xrt = (round(log10(xr(1))):1:round(log10(xr(2)))); end;
        yr  = [10.^floor(log10(1/n)) 1];
